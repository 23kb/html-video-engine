...
(`hyperframes`, `Codex-design`, `after-effects`, `remotion`); every candidate seam gets a strip; every `f-` sheet
...
| `scripts/brief-Codex-design.mjs` | the `Codex-design` target's renderer: contract, OM_SCENES, three motion helpers mapped to `Easing.*`, focus-point camera, seams as cue straddles in both idioms (GSAP on T / animate) |
