...
- Rebuild the real UI from the snapshot's layer manifest (`manifest/layers.json`, real text, PNG crops, `manifest/page@2x.png` at dpr 2; `page-full.png` on newer captures) at the storyboard's mount size; invent no data. Every state a press lands on (a switch ON) needs its own manifest.
- Confirm with one test export that `filter: blur()` survives the svg/foreignObject serialization before building blur-carried seams; if it does not, those seams fall back to opacity crossfades, and the ledger is rebuilt to say so.
