...
- Kit-built mock cards (`mountCardsSpreadFan` defaults, hand-drawn tiles) are only for concepts with **no** real UI (a pre-release feature, an abstract idea). Never as a stand-in when the real capture is sitting in `products/<key>/snapshots/`.
