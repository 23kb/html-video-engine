# Build brief — HyperFrames (GSAP timeline, data-start / data-duration clips) — ref-03
...
| 2.92 | macro crop of the input's left end | fill 0.31 (height) zoom 1.9 | blur push · expo.out | 0.21 | 1.33 | camera | type-on ~16-17 chars/s 3.083-4.417; slow leftward drift (~2 % of width) then an accelerating exit slide left over the last 6 frames | w-2.80:7-11; f-002:27 [high] | Box height 223 px of 720 (frame.mjs 4.0 s) / 118 px wide = 1.89x; heading cap 42 / 28 px = 1.5x (the two landmarks disagree; the box is the cleaner one). Lens move #1. The macro hold also drifts left ~2 % of width (box left edge 200 -> 197 of 480 across frames 76-105) before the exit slide. |
...
| 10.42 | macro crop of the input's left end (cycle 2) | fill 0.31 (height) zoom 1.9 | blur push · expo.out | 0.21 | 1.33 | camera | type-on ~16-17 chars/s 3.083-4.417; slow leftward drift (~2 % of width) then an accelerating exit slide left over the last 6 frames (cycle 2: every time quoted here is +7.5 s) | w-10.30:7-11 [high] | Lens move #2 (twin of 2.917). |
...
|---|---:|---:|---|---|---:|
| S1 | 0.00 | 0.92 | display title the copy on the near-white bed, arriving from under the rightward blue sweep | hard cut | 0.04 s |
| S2 | 0.92 | 0.96 | solid brand-blue field with the white statement the copy | soft-to-sharp resolve | 0.29 s |
| S3 | 1.88 | 1.04 | wide prompt input (placeholder the copy) under the heading the copy, near-white bed | blur push | 0.21 s |
| S4 | 2.92 | 1.54 | macro crop of the same input's left end, prompt typing with a caret; heading the copy cropped at the right edge | hard cut | 0.04 s |
| S5 | 4.46 | 1.04 | right end of the send-state input (the copy) with a blue up-arrow button and a black arrow cursor, on a light-blue gradient bed | melt + settle-in | 0.29 s |
| S6 | 5.50 | 1.21 | white checklist card the copy with three items and tick rings, on the light-blue gradient bed | hard cut | 0.04 s |
| S7 | 6.71 | 0.79 | small bold the copy centred on the near-white bed; soft blue mass at the right edge | colour sweep | 0.33 s |
| S8 | 7.50 | 0.92 | display title the copy on the near-white bed, arriving from under the rightward blue sweep (cycle 2) | hard cut | 0.04 s |
| S9 | 8.42 | 0.96 | solid brand-blue field with the white statement the copy (cycle 2) | soft-to-sharp resolve | 0.29 s |
| S10 | 9.38 | 1.04 | wide prompt input (placeholder the copy) under the heading the copy, near-white bed (cycle 2) | blur push | 0.21 s |
| S11 | 10.42 | 1.54 | macro crop of the same input's left end, prompt typing with a caret; heading the copy cropped at the right edge (cycle 2) | hard cut | 0.04 s |
| S12 | 11.96 | 1.04 | right end of the send-state input (the copy) with a blue up-arrow button and a black arrow cursor, on a light-blue gradient bed (cycle 2) | melt + settle-in | 0.29 s |
| S13 | 13.00 | 1.21 | white checklist card the copy with three items and tick rings, on the light-blue gradient bed (cycle 2) | hard cut | 0.04 s |
| S14 | 14.21 | 0.75 | small bold the copy centred on the near-white bed; soft blue mass at the right edge (cycle 2) | — | — |
...
|---|---|---|---:|---|---|
| linear sweep travel | `none` | (0, 0, 1, 1) | 8 f | the colour sweep mass crossing the frame (~40/480 of width per frame, constant: w-0.00 tiles 1-8) | `none` |
| accelerating exit slide | `power2.in` | (0.11, 0, 0.5, 0) | 5 f | title exit left (4 frames), macro input exit left (6 frames), wide input drift right before the blur push (4 frames) | `power2.in` |
| blur-out | `power3.in` | (0.32, 0, 0.67, 0) | 2 f | outgoing blur on the blur push and the blur-dissolve (2 frames each) | `power3.in` |
| incoming slide settle | `power3.out` | (0.33, 1, 0.68, 1) | 4 f | send-state input settling leftward after the cut at 4.458 (steps 10, 5, 3, 2 of 480); cursor approach deceleration | `power3.out` |
| sharpen-in settle | `expo.out` | (0.16, 1, 0.3, 1) | 4 f | blur push arrival (3 frames), blur-dissolve card (4 frames), title letters behind the sweep edge | `expo.out` |
| fade-in | `power2.out` | (0.5, 1, 0.89, 1) | 7 f | wide input + heading opacity ramp after the cut at 1.875 (curve not resolvable from the ghosted tiles: a guess) | `power2.out` |
| pop / cut-in | `none` | (0, 0, 1, 1) | 1 f | word pops in the statement, the closing line, the send button fill flip | `none` |
| tick clock wipe | `none` | (0, 0, 1, 1) | 3 f | each tick ring fills clockwise from 12 o'clock over 3 frames (~1/3 per frame: f-003 tiles 46-48) | `none` |
