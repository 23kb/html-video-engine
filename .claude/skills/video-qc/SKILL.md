...
2. **Read the report. Map it to a target** via `outline.md`. State the target back if a UI part is named (G2). If the report points at a keyframes contact-sheet tile ("frame 7"), the tile's burned-in `#N t.ts` badge (and the `tools/keyframes.js` stdout tile map) resolves it to an exact second — don't guess which beat it means.
