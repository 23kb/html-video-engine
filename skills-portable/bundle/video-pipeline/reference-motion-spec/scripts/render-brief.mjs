...
switch (target) {
  ...
  case 'hyperframes': {
    ...
    push('## Mapping', '', '- One clip per scene: `data-start` = scene `in`, `data-duration` = `out − in`. A seam spans `[t, t + frames/24]` and the cut (the next scene\'s `in`) sits inside it: the outgoing clip runs to the seam\'s end, the incoming starts at the seam\'s `t`. Both clips are live across the whole seam.',
      '- Base scale: zoom 1 = a real-UI mount at its capture width (one page px = one stage px); the fill values are the reference\'s framing — use the zoom column (derived from fill and the measured box where the spec had none).',
      '- Clips that overlap across a seam sit on different tracks: alternate `data-track-index` 0 / 1 scene by scene, so every seam\'s outgoing and incoming clips are both live.',
