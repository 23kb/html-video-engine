...
// The literal is single-quoted in the host document: an apostrophe inside it ends the string.
...
export const DEFAULT_BLUR = { 'blur-dissolve': 20, 'soft-to-sharp resolve': 16, 'blur push': 24, 'melt + settle-in': 20, 'clear-then-resolve': 16, whip: 12, travel: 8, crossfade: 0 };
export const blurOf = s => { const k = String(s.kind || '').toLowerCase(); if (s.blur_px_1080 != null) return `${s.blur_px_1080}`; const key = Object.keys(DEFAULT_BLUR).find(x => k.includes(x)); return key != null ? `${DEFAULT_BLUR[key]} /* default for ${key} — measure */` : '16 /* default — measure */'; };
export function seamRecipes(kind, cue, a, b, ease, carrier, B = 'B') {
  ...
  if (/blur push|push-through|whip|throw|slide|travel/.test(k)) return [
    `\`tl.to(v.cam, {fx, fy, s, duration: ${dur}, ease: ${e}}, ${at})\` with blur ${B} px riding on the same tween (peak at the cue); the incoming section is already rendered underneath`,
    `\`cam = {fx: animate({from, to, start: ${at}, end: CUES.${cue} + ${b}, ease: ${E}})(T), …}\`; blur = ${B} × the same animate's progress, peaking at the cue`];
  ...
  const ours = carrier ? String(carrier).replace(/\s*\[ref:[^\]]*\]/g, '').trim() : '';
  return [`kind "${kind}": a tween starting at ${at} over ${dur} s${ours ? ` on OUR carrier — ${ours}` : ''}; the mechanics are the ledger row's [ref] outgoing / incoming, applied to our elements`, `an animate({start: ${at}, end: CUES.${cue} + ${b}})(T)${ours ? ` on ${ours}` : ''}, same mechanics`];
}
...
export function renderClaudeDesign(h) {
  ...
  push('## Contract (state once, follow everywhere)', '',
    ...
    '- Real UI: a live iframe cannot sit in the exportable stage. Rebuild each screen from its snapshot\'s layer manifest — the measured boxes and real text ARE the geometry constants a build otherwise hand-types (`manifest/layers.json`, `manifest/raster/`, `manifest/page@2x.png` at dpr 2 as the tracing image; `page-full.png` on newer captures); invent no rows, no data. Confirm with one test export that `filter: blur()` survives the svg/foreignObject serialization before building the blur-carried seams; if it does not, those seams fall back to opacity crossfades and the ledger says so.',
  ...
  push('## OM_SCENES (the outline — write this literal first)', '', '```html', `<script>window.OM_SCENES = '${escQ(JSON.stringify(scenes.map((sc, i) => ({ name: sectionName(sc, i), dur: Number((sc.out - sc.in).toFixed(3)), desc: oneLine(sc.subject) }))))}';</script>`, `<script>window.OM_PLAYBACK = '{"mode":"times","count":1}';</script>`, '```', '', `CUES (authored starts): ${scenes.map((sc, i) => `${sectionName(sc, i)} = ${t3(sc.in)}`).join(' · ')}. Total ${t3(meta.duration)} s. A loop (\`{"mode":"loop"}\`) shows its last frame before its first: settle the choreography by the total and open it at 0.`, '');
  const top = [...eases].sort((a, b) => String(b.used_for || '').length - String(a.used_for || '').length).slice(0, 3);
  push('## Motion helpers (exactly three)', '', '`MOTION = { enter, draw, pop }` are the three motion PATTERNS the file may use (a transform + an opacity shape); each takes `{ ease }` as a parameter, so every ease in the table below is legal inside a helper and none outside one. The three names limit the patterns, not the curves. Default ease per helper:', '');
