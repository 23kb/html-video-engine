#!/usr/bin/env node
// Headed Playwright capture for single-HTML videos — LEGACY #play/#scrub family (klaviyo-bridge-2). For current __tl-contract films use render-singlehtml.js (seek-stepped).
...
//
// Usage: node tools/render-singlehtml-legacy.js <slug> --out <path.mp4> [--bgm bgms/6.mp3] [--duration 53]
...
function parseArgs() {
  ...
  if (!args.slug || !args.out) {
    console.error('Usage: node tools/render-singlehtml-legacy.js <slug> --out <path.mp4> [--bgm <mp3>] [--duration <sec>]');
    process.exit(1);
  }
  return args;
}
